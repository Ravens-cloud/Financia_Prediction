...
def main():
    ...
    # 读取CSV文件
    df = pd.read_csv('weather1.csv', encoding='utf-8-sig')

    # 连接到MySQL数据库
    engine = create_engine('mysql+mysqlconnector://root:root@localhost:3306/text')

    # 将数据存储到MySQL数据库的表中
    df.to_sql('weather1', con=engine, if_exists='replace', index=False)

    # 关闭数据库连接
    engine.dispose()

    # 读取CSV文件
    df = pd.read_csv('weather14.csv', encoding='utf-8-sig')

    # 连接到MySQL数据库
    engine = create_engine('mysql+mysqlconnector://root:root@localhost:3306/text')

    # 将数据存储到MySQL数据库的service_cases表中
    df.to_sql('weather14', con=engine, if_exists='replace', index=False)
